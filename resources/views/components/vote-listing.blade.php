            ...
            <div class="min-w-0 flex-1 flex items-start">
                ...
                <div class="min-w-0 flex-1 px-4">
                    <div>
                        ...
                        <p class="mt-2 text-sm leading-5 text-gray-500">{{ $vote->caster->username . ' ' . __('voted for') . ' ' }} <span class="font-bold">{{ $vote->recipient->name }}</span> <time datetime="{{ $vote->created_at }}">{{ $vote->created_at->diffForHumans() }}</time></p>
                    </div>
                </div>
            </div>
            <div>
