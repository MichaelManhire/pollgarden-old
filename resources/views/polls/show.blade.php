...
<div class="px-4 py-8 bg-white shadow sm:px-10 sm:rounded-lg">
    ...
    @if (Auth::check() && $poll->hasBeenVotedOnBy(auth()->user()->id))
        <table class="block mt-8 max-w-2xl" data-poll-id="{{ $poll->id }}">
    ...
    @else
        <form class="block mt-8 max-w-2xl" id="ballot-box" data-poll-id="{{ $poll->id }}" action="{{ route('votes.store') }}" method="POST">
            @csrf

            <fieldset>
                <legend class="sr-only">{{ $poll->title }}</legend>
                @foreach ($poll->options as $option)
                    <label class="relative flex justify-between pl-12 pr-4 py-4 mb-4 rounded-full bg-gray-300 cursor-pointer" for="{{ $option->id }}">
