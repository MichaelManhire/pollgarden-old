...
<div class="px-4 py-8 bg-white shadow sm:px-10 sm:rounded-lg">
    ...
    <form class="mt-8 max-w-2xl" action="{{ route('votes.store') }}" method="POST">
        @csrf
        ...
        <fieldset>
            ...
            @foreach ($poll->options as $option)
                <label class="relative block pl-12 pr-4 py-4 mb-4 rounded-full bg-gray-300 cursor-pointer" for="option{{ $loop->index }}">
                    <input class="appearance-none fancy-radio-button" id="option{{ $loop->index }}" name="option_id" type="radio" value="{{ $option->id }}">
